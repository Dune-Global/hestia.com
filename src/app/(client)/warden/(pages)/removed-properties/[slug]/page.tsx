...
export default function ApprovedPropertyDetailsPage({
  params,
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      try {
        ...
        console.log(
          "lati from details",
          res.property[0].address.googleMapLocation.latitude
        );
        console.log(
          "longi from details",
          res.property[0].address.googleMapLocation.longitude
        );
        ...
        const landlordFIrstName =
          res.property[0].landlord.fullName.split(" ")[0];
      ...
      <div>
        ...
        <div className="flex flex-col lg:flex-row lg:justify-between lg:gap-10">
          <div className="lg:flex lg:flex-col lg:flex-1">
            <div className="flex gap-2 items-center border-t border-b border-gray-300 py-2 my-2">
              <img
                ...
                alt="profile"
              />
              <div className="text-xs">Hosted by {landlordName}</div>
            </div>
            <div className="py-10">
              <h2 className="font-bold text-xl md:text-2xl md:py-2">
                What this place offers
              </h2>
              <div className="flex flex-col gap-2 md:gap-5 py-3">
                <HostelPerks
                  title="Free Wifi"
                  description="An area with free wifi"
                />
                <HostelPerks
                  title="TV"
                  description="A dedicated TV for your entertainment"
                />
                <HostelPerks
                  title="Air Conditioning"
                  description="Enjoy the breeze"
                />
                <HostelPerks
                  title="Exercise"
                  description="An exercise area with gym equipments"
                />
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-5 items-center justify-center w-full lg:w-1/3 py-8 border-2 rounded-lg border-gray-300">
            <div className="flex flex-col items-center justify-center gap-3">
              <div className="text-gray-400 text-sm">Private room</div>
              <div className="border border-gray-600 rounded-md p-4">
                <div className="text-lg font-semibold">LKR 20, 000</div>
                <div>Per month</div>
              </div>
            </div>
            <div className="flex flex-col items-center justify-center gap-3">
              <div className="text-gray-400 text-sm">Shared room</div>
              <div className="border border-gray-600 rounded-md p-4">
                <div className="text-lg font-semibold">LKR 12, 000</div>
                <div>Per month</div>
              </div>
            </div>
            <div className="flex flex-col gap-4 p-4">
              <Button className="w-full">Approve</Button>
              <Button variant="outlineGray" className="w-full">Reject</Button>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex flex-col gap-2 py-3">
            <p className="text-sm md:text-base">{propertyDescription}</p>
          </div>
        </div>

        <div className="py-2">
          <h2 className="font-bold text-xl md:text-2xl md:py-3">
            Where You Will Be
          </h2>
